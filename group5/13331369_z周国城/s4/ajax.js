(function(){
  var Button, addClickEventToAButtons, addSumToBubble, addClickEventToAllButtons, addTheFetchNumberToSum, addLeaveEventToTheArea, reset, resetForBubble, robot, generateTheRandomListForClicking;
  Button = (function(){
  ...
  addClickEventToAButtons = function(){
    var aButton;
    aButton = $('.apb');
    aButton.click(function(){
      generateTheRandomListForClicking();
    });
  };
  ...
  addClickEventToAllButtons = function(next){
    ...
    function fn$(i, dom){
      new Button($(dom), goodMessages[i], badMessages[i], function(error, number){
        ...
        return typeof next === 'function' ? next() : void 8;
      });
    }
  };
  ...
  robot = {
    initial: function(){
      var i$, ref$, len$, i, radius;
      this.button = $('#button .button');
      this.bubble = $('#info-bar');
      this.numberHasClick = 0;
      this.list = ["A", "B", "C", "D", "E"];
      for (i$ = 0, len$ = (ref$ = $('.info .unread')).length; i$ < len$; ++i$) {
        i = i$;
        radius = ref$[i$];
        $(radius).text('');
      }
    },
    nextToClick: function(){
      if (this.numberHasClick === 5) {
        this.bubble.click();
      } else {
        this.getTheNextButtonToClick().click();
      }
    },
    getTheOrder: function(){
      this.list.sort(function(){
        return 0.5 - Math.random();
      });
    },
    getTheNextButtonToClick: function(){
      var index;
      index = this.list[this.numberHasClick++].charCodeAt() - 'A'.charCodeAt();
      return this.button[index];
    },
    showTheOrder: function(){
      var i$, ref$, len$, i, radius;
      for (i$ = 0, len$ = (ref$ = $('.info .unread')).length; i$ < len$; ++i$) {
        i = i$;
        radius = ref$[i$];
        $(radius).text(this.list[i]);
      }
    }
  };
  generateTheRandomListForClicking = function(){
    robot.getTheOrder();
    robot.nextToClick();
    robot.showTheOrder();
  };
