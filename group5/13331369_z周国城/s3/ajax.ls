class Button
	...
	@all-numbers-get = ->
		...
		true
	@bubble-can-click-when-all-the-numbers-get = !-> bubble.remove-class 'disable' .add-class 'enable'
	@reset-all-buttons = !-> [button.reset! for button in @buttons]
	(@dom, @good-messages, @bad-messages, @next, @number-fetch-back)!->
		...
		@dom.click !~>
			...
			@fetch-number-and-show @name
			@next?!
		@@@buttons.push @

	wait: !-> @state = "waiting"; @dom.remove-class 'enable' .add-class 'waiting'

	fetch-number-and-show: (name)!-> $.get ('/' + name), (number, result) !~>
		@done!
...
add-sum-to-bubble = !->
	...
	Button.bubble-can-click-when-all-the-numbers-get = !-> 
		bubble.remove-class 'disable' .add-class 'enable'
		console.log "fds"
		set-timeout !-> bubble.click!
		, 1000 ##注意的地方
...
add-click-event-to-all-buttons = (next)!->
	...
	for let dom, i in $ '#button .button'
		new Button ($ dom), good-messages[i], bad-messages[i], next, (error, number)->
			if error
...
robot = 
	initial: !->
		...
		@bubble = $ '#info-bar'
		@numberHasClick = 0
	next-to-click: !-> if @numberHasClick < 5
		@button[@numberHasClick++].click!
